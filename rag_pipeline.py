...
llm_model = init_chat_model(model="gpt-4o-mini", temperature=0.2)
...
def create_knowledge_base():
    ...
    #Create vector store from the clunks

    vector_store = Chroma.from_documents(
        documents=docs, embedding=embeddings_model, persist_directory=tempfile.mkdtemp()
    )
    return vector_store
...
and if you don't know the answer, just say "I don't know."""
    ...
    rag_chain = (
        ...
        | prompt
        | llm
        | StrOutputParser() # Convert AIMessage into plain string
    )
